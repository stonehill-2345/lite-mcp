...
class ExternalMCPServiceManager(LoggerMixin):
    ...
    def start_service(self, instance_id: str, proxy_url: Optional[str] = None, 
        ...
        try:
            ...
            # Parse host address
            if config_host and config_host != "null":
                host = config_host
            else:
                host = get_local_ip() or "127.0.0.1"
            ...
            # 7. Update instance configuration (record actually used host and port)
            config_success = external_config_manager.update_instance(
                instance_id,
                {"enabled": True, "host": host, "port": port, "transport": transport}
            )
    ...
    def stop_service(self, instance_id: str, proxy_url: Optional[str] = None, 
        ...
        try:
            ...
            # 5. Modify configuration (disable instance)
            config_success = True
            if disable_config:
                config_success = external_config_manager.disable_instance(instance_id)
                if not config_success:
